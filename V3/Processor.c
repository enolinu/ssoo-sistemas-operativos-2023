...
int registerC_CPU; // General purpose register
int registerD_CPU;
...
void Processor_InitializeInterruptVectorTable(int interruptVectorInitialAddress) {
	...
	interruptVectorTable[CLOCK_BIT]=interruptVectorInitialAddress+4; // CLOCK_BIT=8
}
...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		case HALT_INST: 
			...
			// Solo se apaga si está en modo privilegiado.
		...
		case OS_INST: // Make a operating system routine in entry point indicated by operand1
			...
			if (Processor_PSW_BitState(EXECUTION_MODE_BIT)) {
				...
				ComputerSystem_DebugMessage(130, HARDWARE,InstructionNames[operationCode]
											,operand1,operand2,OperatingSystem_GetExecutingProcessID(), 
...
void Processor_ManageInterrupts() {
	...
	//3 => Si está ya manejando una interrupción, no entra.
	if(!Processor_PSW_BitState(INTERRUPT_MASKED_BIT)) {
		...
		for (i=0;i<INTERRUPTTYPES;i++)
			...
			if (Processor_GetInterruptLineStatus(i)) {
				...
				//Activamos el bit de "manejando interrupción" después de guardar la copia de los registros.
